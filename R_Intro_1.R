...
##  Updated by Yuxuan Wang for her research group members (May 2017)
######################################################################

# To execulate ONE command line, Command + Enter (Mac); Ctrl + R (Windows)
    1+1
    
# Most important command of all: the HELP function: ? or help()
	?mean
...
# Set working directory: (Users: you of course have to set your own working directory; the following is only for my computer...)
	setwd('/Users/yuxuanwang/Documents/Education document/R_Intro_for_Atmos_Sci')
...
# Copy and Paste each command in R to see how it works!
# Note R is case-sensitive, so a and A are different
...
# Binding vectors to form a matrix:
	A = rbind(a, b, d, e)   # Combine by row
	A  # Display A
	B = cbind(a, b, d, e)   # Combine by column
    B  # Display B, note how it is different from A
    
# Matrix:
	C = matrix(1, nrow=3, ncol=5); C   # ";" here is to separate two commands in one line
...
# Loading data:
	load(file='PM25_T_daily_US_201007.RData')   # This file contains several data arrays created in R
	dim(T.US)                                 # T.US is Matrix variable. It is convenient to use "." in variable names
	                                          # T.US stores US surface temperature at 2.5x2.5degree grids for each day of Jul 2010
	head(T.US)                                # Print out the first few values of T.US
	lon.US; lat.US; day                       

# Average over all data points:
	mean(T.US, na.rm=TRUE)                     # Use ?mean to learn what "na.rm" means
# Average over 3rd dimension (day) only to get time average for each lon/lat:
	apply(T.US, c(1,2), mean, na.rm=TRUE)      # Use ?apply to understand its usage
...
# Basic plotting: scatter plot between x and y
...
# E.g. plotting a map for US temperature on 2010-07-01:
	plot(lon.US, lat.US, T.US[,,1])
	map('world', add=TRUE)	

# *** For GEOS-Chem users, the most amazing thing is that in IDL there's a routine "bpch2nc" that can 
	#convert GEOS-Chem outputs into ncdf format, and R has a package "ncdf" that very efficiently handles ncdf files. 
	#I basically do all my GEOS-Chem data analysis in R this way and never really used IDL.
