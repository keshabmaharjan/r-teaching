...
dist.latlon = function(lat1, lon1, lat2, lon2) {
	...
	# 6371.009 km is the mean Earth radius defined by the International Union of Geodesy and Geophysics.
	return(dist)
} 
...
area.latlon = function(lat1, lon1, lat2, lon2) {
	...
	lon1 = lon1/180*pi
	lon2 = lon2/180*pi
	A = abs(6371.009^2*(sin(lat2)-sin(lat1))*(lon2-lon1))	# 6371.009 km is the mean Earth radius defined by the International Union of Geodesy and Geophysics.
	return(A)
}
...
find.lon.lat = function(lonspec, latspec, lon, lat) {
	...
	if (lonspec < (min(lon)-dlon/2) | lonspec > (max(lon)+dlon/2) | latspec > (max(lat)+dlat/2) | latspec < (min(lat)-dlat/2)) return(NaN)
	else {
		if (lonspec < min(lon)) lon1 = 1
		else lon1 = max(which(lon <= lonspec))
		if (lonspec > max(lon)) lon2 = length(lon)
		else lon2 = min(which(lon >= lonspec))
		if (latspec < min(lat)) lat1 = length(lat)
		else lat1 = min(which(lat <= latspec))
		if (latspec > max(lat)) lat2 = 1
		else lat2 = max(which(lat >= latspec))
    	if (abs(lon[lon1]-lonspec) > abs(lon[lon2]-lonspec)) lon.ind = lon2
    	else lon.ind = lon1
    	if (abs(lat[lat1]-latspec) > abs(lat[lat2]-latspec)) lat.ind = lat2
    	else lat.ind = lat1
		...
		for (i in 1:length(lon.frame)) {
			for (j in 1:length(lat.frame)) {
				...
				if (length(ind) < 2)
					...
					stdev[i,j] = sd(spdata[ind], na.rm=TRUE)
				}
			}
		}
	return(stdev)
...
# Dissolving spatial field data into lower resolution (larger pixel size):

sp.dissolve = function(spdata, lon.in, lat.in, lon.out, lat.out, method="mean", dlon.in=NULL, dlat.in=NULL, dlon.out=NULL, dlat.out=NULL) {
	
	# This function dissolves spatial field data (matrix) into lower resolution with larger grid squares.
	# "spdata" is a matrix with rows corresponding to longitudes ("lon.in") and columns corresponding to latitudes ("lat.in").
	...
	# There are two supported methods: 1. default "mean", i.e. calculating the area-weighted average; 2. "mode", take the value occupying the largest area in each output grid.
...
# Dissolving spatial field data into lower resolution (larger pixel size) for 3D data:

sp.dissolve.3D = function(spdata, lon.in, lat.in, lon.out, lat.out, method="mean", full.lon=FALSE, dlon.in=NULL, dlat.in=NULL, dlon.out=NULL, dlat.out=NULL) {
	
# This function dissolves spatial field data (matrix) into lower resolution with larger grid squares for 3D arrays where the 3rd dimension is time.
# "spdata" is a matrix with rows corresponding to longitudes ("lon.in") and columns corresponding to latitudes ("lat.in").
# "lon.out" and "lat.out" are the lon/lat frames for the output resolution.
# "lon.in" and "lon.out" should be in increasing order; "lat.in" and "lat.out" can be in either order, but they should have the same order.
# There is only one supported method, "mean", i.e. calculating the area-weighted average.
# This function utilizes another functions "area.latlon" and "find.lon.lat".
...
# If the data has full longitude range (e.g. 0:360, -180:180), we add matching lon to head and tail of lon.in.
		...
		spdata = abind(spdata[length(lon.in),,], spdata, spdata[1,,], along=1)
		lon.in = lon.in.new
	}
...
moran.I = function(spdata) {
	...
	# This function calculates the Moran's I spatial autocorrelation index for a matrix of spatial data with an adjacency matrix (w_ij = 1 if i and j are adjacent).
	...
	weight.vec <- rep(0, times=length(spdata.vec))
	weight.mat <- matrix(0, nrow=length(weight.vec), ncol=length(weight.vec))
	for (i in 1:length(weight.vec)) {
		for (j in 1:length(weight.vec)) {
			if (i.ind.vec[j] <= i.ind.vec[i]+1 & i.ind.vec[j] >= i.ind.vec[i]-1) {
				if (j.ind.vec[j] <= j.ind.vec[i]+1 & j.ind.vec[j] >= j.ind.vec[i]-1) {
					weight.mat[i,j] <- 1
					}
				else { }
				}
	...
	diag(weight.mat) <- 0
	Y <- na.omit(spdata.vec)
	rm.ind <- as.vector(na.action(Y))
	W <- weight.mat[-rm.ind, -rm.ind]
	I.value <- length(Y)/sum(W)*(t(Y)%*%W%*%Y)/(t(Y)%*%Y)
	return(I.value)
}
...
plot.field = function(spdata, lon.map, lat.map, type=NULL, same=FALSE, zlim=NULL, col=NULL, nlevel=32, mai=c(0.2, 0.2, 0.1, 0.1), mgp=c(1.4, 0.5, 0), tcl=-0.3, ps=12, legend.mar=3, legend.width=1.2, xaxt="n", yaxt="n", Pacific.centric=FALSE, custom.breaks=NULL, map.xlim=NULL, map.ylim=NULL) {
...
# 3. "def": user-defined scale and z-limits. (If chosen, must define the same same scale/limits for all plots.)
# 4. Default: no specification for display.

plot.site = function(spdata, latlon, xlim, ylim, pch=19, type=NULL, zlim=NULL, col=tim.colors(32), nlevel=32, mai=c(0.5, 0.4, 0.2, 0.2), mgp=c(1.4, 0.5, 0), tcl=-0.3, ps=12, legend.mar=3, legend.width=1.2, xaxt="n", yaxt="n") {
	...
	for (i in 1:length(spdata.uniq)) {
		spdata.uniq[i] = mean(spdata[which(latlon[,1] == latlon.uniq[i,1] & latlon[,2] == latlon.uniq[i,2])], na.rm=T)
		}
	if (is.null(type)) zlim = c(min(spdata.uniq, na.rm=T), max(spdata.uniq, na.rm=T))
	else if (type == "frac") zlim = c(0, 1)
	else if (type == "abs") zlim = c(0, max(spdata.uniq, na.rm=T))
	else if (type == "def") zlim = zlim
	else zlim = c(min(spdata.uniq, na.rm=T), max(spdata.uniq, na.rm=T))
	zbreak = seq(zlim[1], zlim[2], length=(length(col) + 1))
	for (i in 1:length(col)) {
		col.uniq[which(spdata.uniq >= zbreak[i] & spdata.uniq <= zbreak[i+1])] = col[i]
		}
	par(mai=mai, mgp=mgp, tcl=tcl, ps=ps)
...
# Quick look at info:
get.info.ncdf = function(file) {
	nc = open.ncdf(file)
	print(nc)
	close.ncdf(nc)
}
# Quick plot (requires functions "plot.field" in "get_geo.R"), and also print out summary statistics:
plot.ncdf = function(file, var.name, dim3=NULL, dim4=NULL, lon=NULL, lat=NULL, lon.name='lon', lat.name='lat', summary.only=FALSE, print.all=FALSE, mfrow=c(1,1), mai=c(0.5, 0.5, 0.2, 0.2), mgp=c(1.4, 0.4, 0), tcl=-0.25, ps=12, type=NULL) {
	...
	nc = open.ncdf(file)
	if (is.null(lon)) lon = get.var.ncdf(nc, lon.name)
	if (is.null(lat)) lat = get.var.ncdf(nc, lat.name)
	if (is.null(dim3) & is.null(dim4)) vars = get.var.ncdf(nc, var.name) else if (!is.null(dim3) & is.null(dim4)) vars = get.var.ncdf(nc, var.name)[,,dim3] else vars = get.var.ncdf(nc, var.name)[,,dim3,dim4]
	...
	if (!summary.only) {
		...
		if (length(dim(vars)) == 2) {
			plot.field(vars, lon, lat, type=type)
		} else if (length(dim(vars)) == 3) {
			for (i in 1:length(dim3)) plot.field(vars[,,i], lon, lat, type=type)
		} else {
			for (i in 1:length(dim3)) {
				for (j in 1:length(dim4)) {
					plot.field(vars[,,i,j], lon, lat, type=type)
				}
			}
		}
	} else { }
